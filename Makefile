...
GITHUB_REPOSITORY = go-pathpatterns
LIBRARY = go-pathpatterns
VERSION = 0.0.1
...
linux: link  ## Build linux binary
	...
	cd ${BUILD_DIR}; \
	GOOS=linux GOARCH=${GOARCH} go build ${LDFLAGS} -buildmode=shared -o ./lib/${LIBRARY}-linux-${GOARCH} . ; \
	cd - >/dev/null

darwin: link  ## Build MacOS binary
	$(info 🔨 Building MacOS binary...)
	cd ${BUILD_DIR}; \
	GOOS=darwin GOARCH=${GOARCH} go build ${LDFLAGS} -buildmode=shared -o ./lib/${LIBRARY}-darwin-${GOARCH} . ; \
	cd - >/dev/null

windows: link  ## Build windows binary
	$(info 🔨 Building Windows binary...)
	cd ${BUILD_DIR}; \
	GOOS=windows GOARCH=${GOARCH} go build ${LDFLAGS} -buildmode=shared -o ./lib/${LIBRARY}-windows-${GOARCH}.exe . ; \
	cd - >/dev/null
